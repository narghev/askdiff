...
Start the askdiff server **and** the browser UI's Vite dev server in the
background, both detached. Pull the parent Claude Code session ID and
project cwd from `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/sessions/$PPID.json`
and pass them in explicitly. After Vite reports its bound port, open the
default browser at the UI URL with the resolved server port wired in via
the `?port=` query parameter.
...
# 4. Start the Vite dev server only if our previous one isn't still alive.
...
ui_url="http://localhost:$vite_port/?port=$port"
